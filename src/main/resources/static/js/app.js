...
function populateDepartmentSelect(selected = []) {
    ...
    ALL_DEPARTMENTS.forEach(dept => {
        if (selected.includes(dept)) return;  // 이미 선택된 건 숨기기
...
// 태그 렌더링
function renderDepartmentTags(depts = []) {
    ...
    depts.forEach(dept => {
        ...
        x.textContent = '×';
        x.onclick = () => removeDepartment(dept);
        tag.appendChild(x);
...
function addDepartment() {
    const sel = departmentSelect.value;
    if (!sel || !currentHospitalId) return;
    const ref = db.collection('hospitals').doc(currentHospitalId);
    ref.update({
        availableDepartments: firebase.firestore.FieldValue.arrayUnion(sel)
    }).then(() => ref.get())
        .then(doc => {
            const list = doc.data().availableDepartments || [];
            renderDepartmentTags(list);
            populateDepartmentSelect(list);
        });
}

// 삭제
function removeDepartment(dept) {
    if (!dept || !currentHospitalId) return;
    const ref = db.collection('hospitals').doc(currentHospitalId);
    ref.update({
        availableDepartments: firebase.firestore.FieldValue.arrayRemove(dept)
    }).then(() => ref.get())
        .then(doc => {
            const list = doc.data().availableDepartments || [];
            renderDepartmentTags(list);
...
auth.onAuthStateChanged(user => {
    if (user) {
        db.collection('hospitals').doc(currentHospitalId).get().then(doc => {
            const list = doc.data().availableDepartments || [];
            renderDepartmentTags(list);
            populateDepartmentSelect(list);
        });
...
auth.onAuthStateChanged(user => {
    if (user) {
        // … 병원 데이터 불러오고 …
        db.collection('hospitals').doc(currentHospitalId).get().then(hospDoc => {
            const list = hospDoc.data().availableDepartments || [];
            renderDepartmentTags(list);
            // 선택된 과목(list)을 제외하고 콤보박스를 다시 채워줌
            populateDepartmentSelect(list);
        });
    }
});
